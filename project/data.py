...
class PolyuDataset(data.Dataset):
    ...
    def __init__(self, root, transforms=None):
        ...
        self.real_images = [fn for fn in files if fn.endswith('real.JPG')]
        self.mean_images = [fn for fn in files if fn.endswith('mean.JPG')]
        assert (len(self.real_images) == len(self.mean_images))

    def __getitem__(self, idx):
        """Load images."""
        image_path = os.path.join(self.root, self.real_images[idx])
        src = Image.open(image_path).convert("RGB")
        image_path = os.path.join(self.root, self.mean_images[idx])
...
def train_data(bs):
    ...
    valid_ds = data.Subset(train_ds, indices[-valid_size:])
    train_ds = data.Subset(train_ds, indices[:-valid_size])
