...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(checkpoint_dir + 'checkpoint.pth.tar',
    ...
    for epoch in range(cur_epoch, 2001):
        ...
        for ind in np.random.permutation(len(train_fns)):
            ...
            for nind in np.random.permutation(len(noise_imgs[ind])):
                ...
                temp_origin_img = origin_imgs[ind][yy:yy + ps_temp, xx:xx +
                                                   ps_temp, :]
                temp_noise_img = noise_imgs[ind][nind][yy:yy + ps_temp, xx:xx +
                                                       ps_temp, :]
                ...
                input_var = torch.from_numpy(temp_noise_img_chw.copy()).type(
                    torch.FloatTensor).unsqueeze(0)
                target_var = torch.from_numpy(temp_origin_img_chw.copy()).type(
                    torch.FloatTensor).unsqueeze(0)
                ...
                if epoch % save_freq == 0:
                    ...
                    temp = np.concatenate(
                        (temp_origin_img, temp_noise_img, output_np), axis=1)
